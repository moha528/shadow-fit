...
export default function RegisterPage() {
  ...
  // Citations sur le défi personnel
  const challengeQuotes = [
    {
      text: "Confronte ton ombre pour révéler ta lumière.",
      author: "Carl Jung",
    },
    {
      text: "Le plus grand adversaire que tu affronteras jamais est celui qui se cache en toi.",
      author: "ShadowFit",
    },
    {
      text: "Chaque répétition est une bataille contre tes limites d'hier.",
      author: "David Goggins",
    },
    {
      text: "Embrasse l'inconfort, car c'est là que la croissance commence.",
      author: "ShadowFit",
    },
    {
      text: "Ta zone de confort est l'ennemie de ton potentiel.",
      author: "ShadowFit",
    },
  ]
  ...
  // Rotation des citations de défi
  useEffect(() => {
    const challengeInterval = setInterval(() => {
      setCurrentChallenge((prev) => (prev + 1) % challengeQuotes.length)
    }, 5000)
      ...
      <div className="login-container flex min-h-screen bg-black overflow-hidden">
        ...
        <div className="hidden rounded-tr-[40px] rounded-br-[40px] lg:block lg:w-1/2 relative bg-zinc-900 overflow-hidden">
          ...
          <div
              ...
              style={{ backgroundImage: 'url("/auth/register.png")' }}
          >
            {/* Overlay gradient */}
            <div className="absolute inset-0 bg-gradient-to-b from-black/30 via-transparent to-black/90 z-0"></div>
          </div>

          {/* Section des défis personnels */}
          <div className="absolute bottom-0 left-0 right-0 p-10 text-white z-20">
            <div className="quote-container backdrop-blur-sm bg-black/20 p-6 rounded-xl border border-white/10 shadow-glow transition-all duration-500">
              <p className="text-2xl font-bold mb-3 leading-tight">"{challengeQuotes[currentChallenge].text}"</p>
              <p className="text-lg text-gray-300 flex items-center">
                <span className="inline-block w-10 h-[1px] bg-gray-400 mr-3"></span>
                {challengeQuotes[currentChallenge].author}
              </p>
            </div>

            <div className="flex mt-6 space-x-2">
              {challengeQuotes.map((_, index) => (
                  <div
                      key={index}
                      className={`h-1 rounded-full transition-all duration-500 ${
                          index === currentChallenge ? "w-10 bg-white shadow-glow" : "w-2 bg-gray-500/50"
                      }`}
                  />
              ))}
            </div>
          ...
          <div className="absolute top-10 left-10 z-20 animate-float">
            <div className="backdrop-blur-md bg-white/10 px-4 py-2 rounded-full border border-white/20 shadow-glow flex items-center">
              ...
              <span className="text-white font-medium">Défiez vos limites</span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
          <div className="max-w-md w-full">
            <div className="flex flex-col items-center mb-10 transition-all duration-300">
              ...
              <p className="text-zinc-400 text-center">Commencez votre voyage vers la meilleure version de vous-même</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center space-x-2 form-field">
                ...
                <Label htmlFor="terms" className="text-sm font-normal text-gray-400">
                  ...
                  <Link
                      href="#"
                      className="text-gray-300 hover:text-white transition-colors duration-300 relative inline-block group"
                  >
